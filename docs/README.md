...
To build the tool for web, run the following command inside `docs` directory:

```
GOOS=js GOARCH=wasm GOEXPERIMENT=jsonv2 go build -o webapp.wasm
```
...
VSCode might give a compiler error in the line `import "syscall/js"` of the file `webapp.go`:
